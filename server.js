        ...
        isInDatabase(username)
            .then((isInDb )=> {
                ...
                if (isInDb) {
                    ...
                    res.end(JSON.stringify({ message: 'Username is already in the database' }));
                    return;
                
                } else {
                    if (email.includes('@') && email.includes('.') && password.length >= 1) {
                        db.run(
                            'INSERT INTO users(username, email, password, isLogged) VALUES(?, ?, ?, 1)',[username, email, password], function (err) {
                                ...
                                res.end(JSON.stringify({ message: 'User was added to the database' }));
                        ...
                        res.writeHead(400, { 'Content-Type': 'application/json' });
                        res.end(JSON.stringify({ message: 'Email or password is not valid' }));
                    }
                }
            })
